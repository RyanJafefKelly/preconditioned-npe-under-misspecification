...
# Submit worker array; pass through selected env knobs
...
OBS_SCENARIO="${OBS_SCENARIO:-misspecified}",\
K="${K:-6}",T="${T:-1000}",\
...
PRECOND_METHOD_DEFAULT="${PRECOND_METHOD_DEFAULT:-smc_abc}",Q_PRECOND="${Q_PRECOND:-0.2}",\
...
SPIKE_STD="${SPIKE_STD:-0.01}",SLAB_SCALE="${SLAB_SCALE:-0.25}",MISSPECIFIED_PROB="${MISSPECIFIED_PROB:-0.5}",\
LEARN_PROB="${LEARN_PROB:-1}",MCMC_WARMUP="${MCMC_WARMUP:-1000}",MCMC_SAMPLES="${MCMC_SAMPLES:-2000}",MCMC_THIN="${MCMC_THIN:-1}",\
THETA_TRUE="${THETA_TRUE:-0.579 -0.143 0.836 0.745 -0.660 -0.254 0.1}",\
THETA_TARGET_OVERRIDE="${THETA_TARGET_OVERRIDE:-}" \
  jobs/svar_worker.pbs
