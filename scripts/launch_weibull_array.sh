...
R=${R:-100}
METHODS_STR=${METHODS_STR:-"npe rnpe pnpe prnpe"}        # local use only
MEM_GB=${MEM_GB:-12}
NCPUS=${NCPUS:-1}
...
# Encode spacey vars as CSV for PBS
METHODS_STR_CSV=$(tr ' ' ',' <<<"$METHODS_STR")
THETA_TRUE_CSV=$(tr ' ' ',' <<<"${THETA_TRUE:-0.8 2.0}")

qsub ${PBS_ARRAY_FLAG} 0-$((TOT-1)) \
  -N weibull_worker \
  -l walltime="${WALLTIME}",mem="${MEM_GB}GB",ncpus="${NCPUS}" \
  -v R="${R}",METHODS_STR_CSV="${METHODS_STR_CSV}",OBS_SEED_OFFSET="${OBS_SEED_OFFSET:-1234}",\
N_OBS="${N_OBS:-200}",OBS_MODEL="${OBS_MODEL:-true}",EPS="${EPS:-0.05}",ALPHA="${ALPHA:-40.0}",\
THETA_TRUE_CSV="${THETA_TRUE_CSV}",THETA_TARGET_OVERRIDE="${THETA_TARGET_OVERRIDE:-}",\
