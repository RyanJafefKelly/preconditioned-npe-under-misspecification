...
# submit worker array; pass through knobs used by the run_* scripts
...
THETA_TRUE="${THETA_TRUE:-0.8 2.0}",THETA_TARGET_OVERRIDE="${THETA_TARGET_OVERRIDE:-}",\
# prior (log-normal on k, lambda)
LOGK_MU="${LOGK_MU:--0.4}",LOGK_SIGMA="${LOGK_SIGMA:-1.0}",\
LOGLAM_MU="${LOGLAM_MU:-$(python - <<'PY'\nimport math;print(math.log(2.0))\nPY)}",\
LOGLAM_SIGMA="${LOGLAM_SIGMA:-0.7}",\
# training size + draws
...
# preconditioning defaults (PNPE/PRNPE use SMC-ABC; NPE/RNPE use none)
Q_PRECOND="${Q_PRECOND:-0.2}",\
SMC_N_PARTICLES="${SMC_N_PARTICLES:-4000}",SMC_ALPHA="${SMC_ALPHA:-0.5}",SMC_EPSILON0="${SMC_EPSILON0:-1e6}",\
...
LEARN_PROB="${LEARN_PROB:-0}",MCMC_WARMUP="${MCMC_WARMUP:-1000}",MCMC_SAMPLES="${MCMC_SAMPLES:-2000}",MCMC_THIN="${MCMC_THIN:-1}" \
