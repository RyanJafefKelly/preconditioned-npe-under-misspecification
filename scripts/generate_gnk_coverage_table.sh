...
JOBID=$(qsub ${PBS_ARRAY_FLAG} 0-$((TOT-1)) \
  ...
  -o logs/gnk_cov_worker.o\$JOB_ID.\$ARRAY_INDEX \
  -e logs/gnk_cov_worker.e\$JOB_ID.\$ARRAY_INDEX \
...
N_SIMS="$N_SIMS",Q_PRECOND="$Q_PRECOND",N_POST="$N_POST",LEVEL="$LEVEL" \
  jobs/gnk_coverage_worker.pbs)
echo "[launcher] array job id: $JOBID"

# ---- submit collate job depending on array completion ----
# PBS Pro: afterokarray; Torque: afterok works on the array root.
DEPEND_OPT="-W depend=afterokarray:$JOBID"
if ! qsub $DEPEND_OPT -N gnk_cov_collate \
  -l walltime=00:10:00,mem=2GB,ncpus=1 \
  -o logs/gnk_cov_collate.o\$JOB_ID \
  -e logs/gnk_cov_collate.e\$JOB_ID \
  -- /bin/bash -lc "uv run python -m precond_npe_misspec.scripts.collate_gnk_coverage \
    --results-root \"$RESULTS_ROOT\" \
    --group-contains \"$GROUP_HINT\" \
    --methods ${METHODS_STR} \
    --level $LEVEL"; then
  # Fallback if afterokarray unsupported
  qsub -W depend=afterok:$JOBID -N gnk_cov_collate \
    -l walltime=00:10:00,mem=2GB,ncpus=1 \
    -o logs/gnk_cov_collate.o\$JOB_ID \
    -e logs/gnk_cov_collate.e\$JOB_ID \
    -- /bin/bash -lc "uv run python -m precond_npe_misspec.scripts.collate_gnk_coverage \
