#!/usr/bin/env bash
# configs/svar/method_matrix.sh
# Central defaults + method/scenario mapping for SVAR experiments.

set -euo pipefail

# ---- Global defaults (override at submit time if needed) ----
export JAX_ENABLE_X64="${JAX_ENABLE_X64:-1}"
export RESULTS_ROOT="${RESULTS_ROOT:-results/svar}"

# Data/model
export K="${K:-6}"
export T="${T:-1000}"
export THETA_TRUE="${THETA_TRUE:-0.579 -0.143 0.836 0.745 -0.660 -0.254 0.1}"

# Target used by metrics (default to true θ; override if you have a pseudo‑true)
...
# Preconditioning
export PRECOND_METHOD_DEFAULT="${PRECOND_METHOD_DEFAULT:-smc_abc}"
...
export DENOISE_MODEL="${DENOISE_MODEL:-spike_slab}"
...
# Optional pseudo‑true vector you previously used (only if you want it)
export THETA_PSEUDOTRUE_DEFAULT="${THETA_PSEUDOTRUE_DEFAULT:-0.835 0.382 0.899 0.824 0.172 0.283 0.1286}"

# Scenario: misspecified | well_specified  (can override per submit)
export OBS_SCENARIO="${OBS_SCENARIO:-misspecified}"

svar_set_method_env() {
  local method="$1"
  # Scenario → obs_model
  local obs_model="true"; if [[ "${OBS_SCENARIO}" == "well_specified" ]]; then obs_model="assumed"; fi

  # Base across all methods
  export THETA="${THETA_TRUE}"
  export OBS_MODEL="${OBS_MODEL:-$obs_model}"

  case "$method" in
    npe)
      export PRECOND_METHOD="none"
      export POSTERIOR_METHOD="npe"
      # Default target: true θ. If you want pseudo‑true for comparisons, set THETA_TARGET_OVERRIDE.
