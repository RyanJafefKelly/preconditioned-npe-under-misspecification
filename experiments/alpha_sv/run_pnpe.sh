...
# Preconditioning (PNPE): default to rejection ABC
: "${N_SIMS:=20000}"; : "${Q_PRECOND:=0.2}"; : "${PRECOND_METHOD:=rejection}"
# SMC params (used if PRECOND_METHOD=smc_abc)
: "${SMC_N_PARTICLES:=1000}"; : "${SMC_ALPHA:=0.5}"; : "${SMC_EPSILON0:=1e6}"
...
GROUP="th_${THETA_TAG}-T_${T}-th1_${THETA1}-obs_${OBS_MODEL}-pre_${PRECOND_METHOD}-q_${Q_PRECOND}-n_sims_${N_SIMS}"
OUTDIR="results/alpha_sv/pnpe/${GROUP}/seed-${SEED}/${DATE}"; mkdir -p "$OUTDIR"
